...
const OrderInvoiceModal = ({ isOpen, setIsOpen, invoiceInfo, items }) => {
  function closeModal() {
    setIsOpen(false);
  }
    ...
    toPng(dom)
      .then((dataUrl) => {
        ...
        img.onload = () => {
          ...
          // Output / Save
          pdf.save(`invoice-${invoiceInfo.orderNumber}.pdf`);
        };
      })
      .catch((error) => {
                ...
                <div className="mt-6">
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody>
                      {items.map((item) => (
                        <tr key={item.orderId}>
                          <td className="w-full">{item.orderName}</td>
                          ...
                          <td className="min-w-[90px] text-right">
                            {currencySymbol}
                            {Number(item.orderValue * item.qty).toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
